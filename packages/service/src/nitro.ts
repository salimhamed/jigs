...
// Nitro resolves a bare `plugins` entry against the build root, which is now
// the factory rather than this package, so the path has to be absolute and
// resolved from this module.
const startWorldPlugin = fileURLToPath(
  new URL("../plugins/start-world.ts", import.meta.url),
);

export interface JigsServiceOptions {
  /** Entry route handler, relative to the factory root. Defaults to the one
   *  `prepare()` generates. */
  entry?: string;
}
...
export function defineJigsService(
  options: JigsServiceOptions = {},
): NitroConfig {
  ...
  return {
    ...
    routes: {
      "/**": options.entry ?? `./${GENERATED_DIR}/${GENERATED_ENTRY_FILE}`,
    },
  };
}
