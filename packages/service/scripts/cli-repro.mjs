...
const { env } = await startLinearGithubStub();
const PG_URL =
...
// Swept before the seed as well as after it: `assert` exits the process, so a
// failure anywhere past this point skips the trailing cleanup and would leave
// a phantom row in every later `jigs ps` until the next *successful* run.
await sql`DELETE FROM jigs_worktrees WHERE path LIKE '/tmp/jigs-cli-repro/%'`;
await sql`
