...
//
// No `app_mention` event and no `app_mentions:read`: Slack delivers a channel
// mention as both an app_mention and a message.channels, so subscribing to
// both is two deliveries of one utterance. A mention arrives as an ordinary
// message whose text holds the bot's id.
...
# the factory repo's .env, beside the key the agent's model calls are billed to:
#
...
#
# Then declare the channel, the allowlist and the model under \`slack:\` in
# jigs.yml and restart the service.`;
