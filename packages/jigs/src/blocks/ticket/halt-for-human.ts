...
/** Interpolated into a prompt or a comment template; never rendered as one. */
 ...
 * `headline` is one plain sentence naming what jigs paused and why, `about`
 * restates the ticket itself, `notes` are plain bullet lines, and `onReply`
 * decides what the comment asks the human to do: choose between the questions
 * ("continue") or repair something and let the step run again ("retry").
 */
export type Halt = {
  headline: string;
  about?: string;
