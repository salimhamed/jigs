...
function footer(context: NeedsHumanContext): string {
  const parts = [`Run ${context.runId}`];
  if (context.pipeline !== undefined && context.pipeline !== "") {
    parts.push(`pipeline \`${context.pipeline}\``);
  }
  parts.push(`paused at ${context.pausedAt}`);
