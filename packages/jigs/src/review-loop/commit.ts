// Saving work the builder finished but never committed (ADR 0009): the same
// resume-first shape the CI fix and the review answers use, with the same
// prompt on both arms — the work is on disk in the cwd, so a fresh builder
// reading the worktree has everything the resumed one would have had.
