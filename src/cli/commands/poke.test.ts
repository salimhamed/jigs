...
test("a 404 becomes a JigsError naming the run", async () => {
  ...
  await expect(pokeRun("wr_missing", deps())).rejects.toThrow("run wr_missing not found");
});
...
test("a trailing slash on the service URL does not break the poke route", async () => {
  fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ runId: "wr_abc", poked: [] })));
