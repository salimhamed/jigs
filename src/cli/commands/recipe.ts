...
export function addRecipe(name: string, deps: { cwd: string; out: (line: string) => void }) {
  ...
  deps.out(
    "Then run jigs build, pnpm typecheck, and pnpm test. Copied recipe files are yours to edit.",
  );
  return result;
}
