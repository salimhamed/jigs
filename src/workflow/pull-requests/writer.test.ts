...
import { currentRunId, defaultPullRequestScope } from "./writer.ts";
...
test("the default scope is the workflow's function name, not where its file sits", () => {
  expect(defaultPullRequestScope("AGE-123")).toBe("shipWorkflow/AGE-123");
  ...
  expect(defaultPullRequestScope("AGE-123")).toBe("deliverWorkflow/AGE-123");
