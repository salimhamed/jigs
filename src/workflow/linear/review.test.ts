...
test("the needs-human round re-reads the ticket, so the human's reply is what the next review sees", async () => {
  ...
  // the loop is inside this routine rather than in every workflow that calls it.
...
test("a caller-supplied prompt replaces the one shipped beside the routine", async () => {
  verdicts = [proceed()];
