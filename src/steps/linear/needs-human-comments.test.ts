...
test("a note is created under an id derived from the run, the issue and the note", async () => {
  await postTicketNote("issue-1", done, context, definition);
  const id = ticketCommentId(context.workflowRunId, "issue-1", { note: done });
  ...
  expect(ticketCommentId("wrun_other", "issue-1", { note: done })).not.toBe(id);
  expect(
    ticketCommentId(context.workflowRunId, "issue-1", { note: { ...done, notes: ["x"] } }),
  ).not.toBe(id);
});

test("a note's id ignores who it mentions, so a changed operator cannot post it twice", async () => {
  await postTicketNote("issue-1", done, context, definition);
...
test("a retried halt question is found under the same id instead of posted twice", async () => {
  ...
  expect(findComment).toHaveBeenCalledWith(
    ticketCommentId(context.workflowRunId, "issue-1", { halt: questions }),
  );
  expect(createComment).not.toHaveBeenCalled();
});
