...
picks one). Under the newly installed CLI it regenerates `jigs.ts` and runs
`jigs up` to build and start the factory, then runs the factory's own typecheck. This is a single command even
